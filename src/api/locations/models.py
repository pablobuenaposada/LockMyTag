...
class Tag(TimeStampedModel):
    id = models.CharField(
        ...
        unique=True,
        primary_key=True,
        help_text="Unique identifier for the AirTag",
    )
...
class TagLocation(TimeStampedModel):
    tag = models.ForeignKey(
        Tag, on_delete=models.CASCADE, help_text="Associated AirTag"
    )
    latitude = models.DecimalField(
        max_digits=9, decimal_places=6, help_text="Latitude of the location"
    )
    longitude = models.DecimalField(
        max_digits=9, decimal_places=6, help_text="Longitude of the location"
    )
